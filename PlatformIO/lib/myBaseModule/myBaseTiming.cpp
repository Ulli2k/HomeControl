...
void myTiming::delay(unsigned long ms) {
	uint16_t start = (uint16_t)myTiming::micros();

	while (ms > 0) {
		if (((uint16_t)myTiming::micros() - start) >= 1000) {
			ms--;
			start += 1000;
		}
	}
...
void myTiming::delayMicroseconds(unsigned int us) {
...
void myTiming::initialize() {

	configure();
	cTimeCallbacks=0;
}

void myTiming::attachTimeInterrupt(void (*isr)(), unsigned long milliseconds)
{
	...
	//TimeCallbacks[cTimeCallbacks].created = myTiming::millis();
	cTimeCallbacks++;
}

void myTiming::detachTimeInterrupt(void (*isr)())
{
