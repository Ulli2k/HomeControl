...
typedef struct {
  ...
  uint8_t DataTypeCode; // RFM: Decoder/Encloder Type ID
  uint8_t Data[MAX_RING_DATA_SIZE];  // RFM: 25 , Uart: 25(Vorsicht \0 -->+1), IR: 15
  uint8_t DataSize;
} RecvData;
...
#define FIFO_init(fifo)		{ fifo._read = 0; fifo._write = 0; fifo._ready = 1; }
